            ...
            for r in restrooms:
                try:
                    ...
                    _, was_created = Amenity.objects.update_or_create(
                        ...
                        defaults={
                            'name': str(r.get('facility_name') or 'Public Restroom')[:200],
                            'latitude': lat,
                            'longitude': lon,
                            'operator': str(r.get('operator', ''))[:200],
                            'hours_of_operation': str(r.get('hours_of_operation', ''))[:500],
                            'changing_stations': str(r.get('changing_stations', '')).lower() in ['true', 'yes', '1'],
                            'accessibility': accessibility,
                            'active': str(r.get('status', 'true')).lower() in ['true', 'yes', '1', 'open', 'available', 'operational'],
                        }
                    )
                    if was_created:
                        created += 1
