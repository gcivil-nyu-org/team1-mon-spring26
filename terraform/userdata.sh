...
# pull from ssm: /etc/pki/tls/certs/server.crt
aws ssm get-parameter --name "/nycnow/env/ssl/server.crt" --with-decryption --query "Parameter.Value" --output text --region $REGION > /etc/pki/tls/certs/server.crt

# pull from ssm: /etc/pki/tls/certs/server.key
aws ssm get-parameter --name "/nycnow/env/ssl/server.key" --with-decryption --query "Parameter.Value" --output text --region $REGION > /etc/pki/tls/certs/server.key

chmod 400 /etc/pki/tls/certs/server.key
# /etc/pki/tls/certs/server.crt is public
