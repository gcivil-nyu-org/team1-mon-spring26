...
class Command(BaseCommand):
    help = 'Import LinkNYC kiosks by combining kiosk status and NYC Wi-Fi hotspot datasets'
    ...
    def _normalize_status_row(self, kiosk):
        ...
        address_components = [part for part in [address, city, boro if boro != city else ''] if part]
    ...
    def _normalize_wifi_row(self, hotspot):
        ...
        is_linknyc = 'linknyc' in provider_lower or source_id.upper().startswith('LINK-')
        if not is_linknyc:
            return None
        ...
        display_name = f'LinkNYC {source_id}' if source_id else f'LinkNYC {name}' if name else 'LinkNYC Kiosk'
        ...
        address_components = [part for part in [location_text, city, boroname if boroname != city else ''] if part]
    ...
    def _merge_record(self, merged_record, candidate):
        if candidate['priority'] > merged_record['priority']:
            for key in ['name', 'address', 'prop_name', 'description', 'active', 'provider', 'source_label', 'priority']:
        ...
        merged_record['active'] = bool(merged_record.get('active', True) and candidate.get('active', True))
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(f'Found {len(status_rows)} rows in LinkNYC status dataset')
            ...
            for row in status_rows:
                ...
                if not record:
                    continue
                key = f"id:{record['canonical_id']}" if record['canonical_id'] else f"coord:{record['coord_key']}"
            ...
            for row in wifi_rows:
                ...
                key_by_id = f"id:{record['canonical_id']}" if record['canonical_id'] else ''
                target_key = ''
            ...
            with transaction.atomic():
                for record in merged_by_primary_key.values():
                    try:
                        ...
                        self.stdout.write(self.style.WARNING(f'Skipped entry: {e} - {record}'))
                        skipped_count += 1
                        continue
