...
def parse_hours(hours_string):
    """
    Parses a human-readable hours string into a structured dictionary with 24-hour times.
    """
    if not hours_string or not isinstance(hours_string, str) or hours_string.strip().lower() in ['n/a', 'not operational', 'temp closed']:
    ...
    if 'by permit' in hours_string.lower() or hours_string.lower().startswith('http') or 'see website' in hours_string.lower():
    ...
    day_time_regex = re.compile(
        ...
        r'\s*[:\-]?\s*'  # Separator
        r'([\w\s:./\-,&]+?(?:\d{1,2}(?::\d{2})?\s*(?:am|pm)?|dusk|closed|dawn))'  # Time string
        , re.IGNORECASE
    )
    ...
    day_time_regex = re.compile(
        r'\b(M|T|W|Th|F|S|Su|Mon|Tues|Wed|Thurs|Fri|Sat|Sun|Weekdays|Weekends|Daily)s?\b'  # Day(s)
        r'(?:\s*(-|–|to)\s*\b(M|T|W|Th|F|S|Su|Mon|Tues|Wed|Thurs|Fri|Sat|Sun)\b)?'  # Optional day range
        r'\s*[:\-]?\s*'
        r'((?:\d{1,2}(?::\d{2})?\s*(?:am|pm)?|dusk|closed|dawn)(?:\s*(-|–|to)\s*(?:\d{1,2}(?::\d{2})?\s*(?:am|pm)?|dusk|dawn))?)'
        , re.IGNORECASE
    )
    ...
    # --- Logic for single-line formats (including complex ones like "M-T ... W-S ...") ---
    ...
    if matches:
        ...
        for match in matches:
            if match.start() > last_end:
                unparsed_parts.append(processed_string[last_end:match.start()].strip(',. '))
            last_end = match.end()
            ...
            if start_day_lower == 'weekdays':
                ...
                try:
                    ...
                    # Handle ranges that wrap around the week, e.g., S-Su (Sat-Sun) or F-M (Fri-Mon)
            ...
            if 'closed' in time_str.lower():
                ...
                time_matches = re.findall(r'(\d{1,2}(?::\d{2})?\s*(?:am|pm)?|dusk|dawn)', cleaned_time_str, re.IGNORECASE)
                if len(time_matches) >= 2:
                    open_time, close_time = _parse_time(time_matches[0]), _parse_time(time_matches[1])
        ...
        time_matches = re.findall(r'(\d{1,2}(?::\d{2})?\s*(?:am|pm)?|dusk|dawn)', main_part, re.IGNORECASE)
        if len(time_matches) == 2:
            open_time, close_time = _parse_time(time_matches[0]), _parse_time(time_matches[1])
            if open_time and close_time:
