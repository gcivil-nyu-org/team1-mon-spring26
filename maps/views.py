...
def cluster_amenities(amenities, zoom):
    ...
    # This raw query is more efficient for grid-based clustering than the ORM equivalent.
    ...
    with connection.cursor() as cursor:
        cursor.execute(cluster_query, [grid_size, list(amenities.values_list('id', flat=True))])
...
def amenities_api(request):
    ...
    amenities = amenities.select_related('amenity_type').prefetch_related('reviews__user')


    # --- Backend Clustering Logic ---
    BIKE_RACK_TYPE_NAME = 'Bike Rack'
    CLUSTER_ZOOM_THRESHOLD = 18 # Cluster below this zoom level. Increase to cluster at higher zoom levels.
    ...
    if is_bike_rack_query and zoom < CLUSTER_ZOOM_THRESHOLD:
        ...
        single_bike_racks = bike_rack_amenities.filter(id__in=single_point_ids) # This queryset already has annotations
