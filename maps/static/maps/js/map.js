    ...
    fetch('/api/reviews/', {
        ...
        .then(({ s, b }) => {
            ...
            const newReview = {
                ...
                upvote_count: Number(b.upvote_count || 0),
                downvote_count: Number(b.downvote_count || 0),
...
function renderVoteControls(review, loggedIn, currentEmail, featured = false) {
    ...
    const upvoteTitle = (isOwn || !loggedIn) ? title : upvoteLabel;
    const downvoteTitle = (isOwn || !loggedIn) ? title : downvoteLabel;
    const upvoteCountHtml = `<span class="review-vote-count ${upvoteCount > 0 ? '' : 'is-empty'}" data-review-upvotes="${reviewId}" aria-label="${upvoteLabel}" aria-hidden="${upvoteCount > 0 ? 'false' : 'true'}">${upvoteCount}</span>`;
    const downvoteCountHtml = `<span class="review-vote-count ${downvoteCount > 0 ? '' : 'is-empty'}" data-review-downvotes="${reviewId}" aria-label="${downvoteLabel}" aria-hidden="${downvoteCount > 0 ? 'false' : 'true'}">${downvoteCount}</span>`;
    const optionClass = `review-vote-option${isOwn ? ' is-readonly' : ''}`;
    const thumbsUpIcon = userVote === 1
    ...
    const thumbsDownIcon = userVote === -1
    ...
    return `<div class="${rowClass}" data-review-vote-row="${reviewId}">
        <span class="${optionClass}" title="${upvoteTitle}">
            <button type="button" class="review-vote-btn js-vote-btn up ${userVote === 1 ? 'active' : ''}" data-review-id="${reviewId}" data-vote="1" ${isOwn ? 'disabled' : ''} title="${upvoteTitle}" aria-label="Upvote review" aria-pressed="${userVote === 1}">${thumbsUpIcon}</button>
            ${upvoteCountHtml}
        </span>
        <span class="${optionClass}" title="${downvoteTitle}">
            <button type="button" class="review-vote-btn js-vote-btn down ${userVote === -1 ? 'active' : ''}" data-review-id="${reviewId}" data-vote="-1" ${isOwn ? 'disabled' : ''} title="${downvoteTitle}" aria-label="Downvote review" aria-pressed="${userVote === -1}">${thumbsDownIcon}</button>
            ${downvoteCountHtml}
        </span>
    </div>`;
}
...
function updateReviewVoteControls(amenity, review, pane, row) {
    if (!row) return;

    const featured = row.classList.contains('rv-review-vote-row');
    const loggedIn = currentUser && currentUser.is_authenticated;
    const currentEmail = (currentUser?.email || '').toLowerCase();
    row.outerHTML = renderVoteControls(review, loggedIn, currentEmail, featured);

    const updatedRow = pane.querySelector(`[data-review-vote-row="${Number(review.id || 0)}"]`);
    if (!updatedRow) return;

    updatedRow.querySelectorAll('.js-vote-btn').forEach(btn => {
        btn.addEventListener('click', () => submitReviewVote(amenity, pane, btn));
    });
}
...
function submitReviewVote(amenity, pane, clickedButton) {
    ...
    fetch(`/api/reviews/${reviewId}/vote/`, {
        ...
        .then(({ s, b }) => {
            ...
            review.upvote_count = Number(b.upvote_count || 0);
            review.downvote_count = Number(b.downvote_count || 0);
            review.user_vote = Number(b.user_vote || 0);
            updateReviewVoteControls(amenity, review, pane, row);
        })
        .catch(() => {
        ...
        .finally(() => {
            if (pane.querySelector(`[data-review-vote-row="${reviewId}"]`) === row) {
                buttons.forEach(button => {
                    button.removeAttribute('aria-disabled');
                });
            }
...
function setupAuthForm(formId, url, errId, isReg = false) {
    document.getElementById(formId).addEventListener('submit', e => {
        ...
        const pass = e.target.querySelector('input[name="password"], input[type="password"]').value;
        const confirmInput = e.target.querySelector('input[name="confirm_password"]');
        const confirmPass = confirmInput ? confirmInput.value : pass;

        if (isReg && pass !== confirmPass) {
            showAuthError(errId, 'Password and confirmation do not match');
            return;
        }
